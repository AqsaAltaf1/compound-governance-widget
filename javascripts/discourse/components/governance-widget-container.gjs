  ...
  renderWidgets(element) {
    ...
    this.proposals.forEach((proposal, index) => {
      if (renderingUrls.has(proposal.url)) {
        console.log(
          `🔵 [COMPONENT] URL ${proposal.url} is already being rendered, skipping duplicate`
        );
        return;
      }
        ...
        renderMultiStageWidget(
          {
            ...
            arfc:
              stage === "arfc" || stage === "snapshot" ? proposal.data : null,
            arfcUrl:
              stage === "arfc" || stage === "snapshot" ? proposal.url : null,
            aip: null,
            aipUrl: null,
          },
          widgetId,
