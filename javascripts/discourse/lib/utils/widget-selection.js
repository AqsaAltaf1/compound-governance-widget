...
export function selectTopProposals(proposalsList) {
  ...
  const allTypes = sorted.map(p => {
    ...
    return proposalType === 'temp-check' ? 'tempcheck' : 
  ...
  console.log(`🔵 [SELECTION] Selected ${selected.length} proposal(s) from ${proposalsList.length} total (${isSingleType ? 'single type - showing all 3' : 'multiple types - max 2 per type'}):`);
